...
test('Decode post metadata', () => {
  expect(
    ...
    title: 'Spolupráce s Česko.Digital',
    authorId: 'marek',
    coverImageUrl: 'https://data.cesko.digital/img/clanek-pohyb-je-reseni/2.png',
    date: '2022-01-10-01-25',
    slug: 'pribeh-inkubacniho-procesu',
    description: 'Příběh tříměsíčního designového procesu…',
    lang: 'cs',
...
test('Parse post path', () => {
  expect(parsePath('2022-01-26-cist-digital-30.md')).toEqual([new Date('2022-01-26'), 'cist-digital-30']);
...
test('Parse all posts', () => {
  const postPaths = getFilesRecursively('content/posts').filter((path) => path.endsWith('.md'));
